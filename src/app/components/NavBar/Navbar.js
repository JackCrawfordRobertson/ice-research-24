...
const Navbar = ({ setCurrentSlide, currentSlide }) => {
  ...
  return (
    <AppBar
      ...
      sx={{
        backgroundColor: '#ffffff',  // Solid grey background
      ...
      <Toolbar style={{ justifyContent: 'center' }}>
        {!isMobile && (
          ...
          sx={{
            ...
            fontSize: '.8rem',
            padding: '12px 12px',
            margin: '0 2px',  // Add horizontal margin for spacing between tabs
            borderRadius: '8px',
            transition: 'all 0.3s ease',  // Smooth transition for background and text color
            color: currentTab === index ? '#333' : '#333',  // White text for selected tab, black for unselected
            ...
            '&:hover': {
              ...
              color: '#333', // Keep text white on hover when selected
            },
            '&.Mui-selected': {
